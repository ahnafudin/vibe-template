...
//   2. beads init — REFUSES on a dirty index (`bd init` auto-commits every
//      staged file, a real data-loss footgun)
//   3. `bd setup claude` — SessionStart/PreCompact `bd prime` hooks
//   4. Dolt sync remote = the git origin (lives in the LOCAL beads DB, so this
//      step repeats on every new machine)
...
if (!tryRun("git", ["rev-parse", "--is-inside-work-tree"]).ok) {
...
if (where.ok && where.out.includes(".beads")) {
  note("workspace already active — skipping `bd init`.");
} else {
  // GUARD: `bd init` auto-commits everything staged. Refuse on a dirty index.
...
if (check.ok && !/✗|not installed|No hooks/i.test(check.out)) {
  ...
  note("NOTE: if bd rewrote the Beads section in CLAUDE.md, restore the reconciled");
  note("rules from this template (no auto-push; docs/TASKS.md stays the roadmap).");
}
